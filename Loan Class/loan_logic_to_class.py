...
loan_list = [WF1_Loan,Lisa_Car,WF2_Loan,mortgage]
...
def get_payments(perms, extra_money):
    ...
    payment_dict = {}
    for perm_item in range(len(payoff_dict)):

        test_dict = {}
        for iter,num in enumerate(payoff_dict[perm_item]):
            temporary_list = [] # create temp list
            payment_amount = perms[perm_item][iter][2] #ggrab payment for that loan
            ...
            payment_dict[perm_item] = test_dict
...
def Loan_payoff(perms,extra_money,payoff_style):
    ...
    for perm_item in range(len(perms)):
        ...
        for loan_attr in range(len(perms[perm_item])):

            principal = perms[perm_item][loan_attr][0]
            interest_rate = perms[perm_item][loan_attr][1]
            name = perms[perm_item][loan_attr][3]
            ...
            inner_loop_index =[]
            inner_index += 1
            nper = payoff_dict[perm_item][loan_attr]
            upper = math.ceil(nper)
            period = list(range(upper))

            for per in range(upper):
                if loan_attr == 0:
                    monthly_payment = payoff_var[perm_item][loan_attr][per] + extra_money
                else:
                    monthly_payment = payoff_var[perm_item][loan_attr][per]
                if prin[-1] > 0:
